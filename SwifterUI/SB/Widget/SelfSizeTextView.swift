...
open class SelfSizeTextView: PlaceholderTextView {
    
    public var didIncreaseHeight: ((CGFloat) -> Void)?
    
    public var maxLines: Int = 6 { didSet { updateSizeIfNeededAnimated(false) } }
    
    fileprivate let estimationLayoutManager = NSLayoutManager()
    fileprivate let estimationTextContainer = NSTextContainer()
    
    public var heightConstraint: NSLayoutConstraint {
        guard let constraint = constraints.first(where: { $0.firstAttribute == .height && $0.relation == .equal }) else {
